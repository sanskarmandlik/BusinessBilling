...
title Ledgerly - Launcher
color 0A
echo.
echo  ====================================================
echo   LEDGERLY - Starting All Services...
echo  ====================================================
echo.
...
netsh advfirewall firewall show rule name="Ledgerly Port 5000" >nul 2>&1
if %errorlevel% NEQ 0 (
    netsh advfirewall firewall add rule name="Ledgerly Port 5000" dir=in action=allow protocol=TCP localport=5000 >nul 2>&1
...
start "Ledgerly - Backend API" cmd /k "cd /d "%~dp0backend" && color 0B && npm run dev"
...
start "Ledgerly - Public Tunnel" cmd /k "color 0D && npx localtunnel --port 5000 --subdomain sannabilling"
...
start "Ledgerly - Frontend UI" cmd /k "cd /d "%~dp0frontend" && color 0E && npm run dev -- --host"
